...
simulateStrategy <- function (symbol){
  ...
  transactions <- lapply(startPoints, function(s){
    ...
    results <- c(coredata(sp), coredata(ep), index(ep)-index(sp), yield, yieldp, targetPriceReached)
    names(results) <- c("startPoint","endPoint","timeFrame","yield","yieldRatio","tpr")
    results
  })
  ...
  png(sprintf("plots/%s_%s.png", (cash-1)*100, symbol), width = 1920, height = 1080)
  par(mfrow = c(2,1))
  ...
  transactions$yield = as.numeric(cash)
  transactions
}
...
for(symbol in symbols){
  transactions <- simulateStrategy(symbol)
  #results <- append(results,  transactions$yield)
}
